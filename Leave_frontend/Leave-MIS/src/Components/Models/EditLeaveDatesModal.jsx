...
const EditLeaveDatesModal = ({ show, onHide, leave, onSave }) => {
  ...
  const isShort    = leave?.leaveType === "SHORT" || leave?.leaveType === "SHORT_LEAVE";
  const isHalfDay  = leave?.leaveType === "HALF_DAY";
  ...
  // ── When half-day period changes, offer sensible default times ──
  const handlePeriodChange = (period) => {
    setHalfDayPeriod(period);
    const defaults = HALF_DAY_DEFAULTS[period];
    if (defaults) {
  ...
  const handleSave = async () => {
    ...
    setSubmitting(true);
    try {
      await onSave(leave.id, payload);
      onHide();
    } catch (err) {
      setError(err?.response?.data || err?.message || "Failed to update leave. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const canSave =
    ...
    (isShort
      ...
      : startDate && endDate && (workingDaysInfo?.workingDays ?? 1) > 0);
      ...
      >
        ...
        <div className="d-flex align-items-center justify-content-between p-3 p-md-4 border-bottom">
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              <h5 className="mb-0 fw-bold text-dark" style={{ fontSize: "1rem" }}>
                {isShort ? "Edit Leave Time" : "Edit Leave Dates"}
              </h5>
              <small className="text-muted">{leaveTypeLabel}</small>
            </div>
          </div>
        ...
        <div className="p-3 p-md-4">
          ...
          <div
            ...
            style={{ background: "#f8f9fa", border: "1px solid #e9ecef" }}
          >
            <div className="small text-muted fw-semibold mb-1">Current Details</div>
            <div className="fw-bold text-dark" style={{ fontSize: "0.95rem" }}>
              {fmtDate(leave.startDate)}
              {isRegular && (
                <span className="text-muted fw-normal"> → {fmtDate(leave.endDate)}</span>
              )}
            </div>
            {isShort && (
              <div className="small text-muted mt-1 d-flex align-items-center gap-1">
                <Clock size={11} />
                {fmtTime(leave.shortLeaveStartTime)} – {fmtTime(leave.shortLeaveEndTime)}
              </div>
            )}
            {isHalfDay && (
              <div className="small text-muted mt-1 d-flex align-items-center gap-1">
                <Clock size={11} />
                {leave.halfDayPeriod || "MORNING"} · {fmtTime(leave.halfDayStartTime)} – {fmtTime(leave.halfDayEndTime)}
              </div>
            )}
            {isRegular && (
              <div className="small text-muted mt-1 d-flex align-items-center gap-1">
          ...
          {/* ── SHORT LEAVE: time only ── */}
          {isShort && (
            <div className="mb-3">
              <div className="small fw-semibold text-dark mb-2">New Time</div>
              <div className="row g-3">
                <div className="col-6">
                  <label className="form-label small text-muted mb-1">
          ...
          {isHalfDay && (
            <>
                  ...
                  {["MORNING", "AFTERNOON"].map((p) => (
                    <button
                      ...
                      className={`btn btn-sm rounded-3 flex-fill ${
                        halfDayPeriod === p ? "btn-primary" : "btn-outline-secondary"
                      }`}
              ...
              <div className="mb-3">
                <div className="small fw-semibold text-dark mb-2">Time</div>
                <div className="row g-3">
                  <div className="col-6">
                    <label className="form-label small text-muted mb-1">
                      Start Time <span className="text-danger">*</span>
              ...
              {startDate && endDate && (
                <div className="mb-3">
                  {calculating ? (
                    ...
                    <div
                      className={`rounded-3 px-3 py-2 small ${
                        workingDaysInfo.workingDays === 0 ? "border border-warning" : ""
                      }`}
                      style={{
                        background:
                          workingDaysInfo.workingDays === 0
                    ...
                    >
                      <div className="d-flex flex-wrap gap-3 align-items-center">
                        <span
                          className={`fw-bold ${
                            workingDaysInfo.workingDays === 0 ? "text-warning" : "text-success"
                          }`}
                        >
                          {workingDaysInfo.workingDays} working day
                          {workingDaysInfo.workingDays !== 1 ? "s" : ""}
                        ...
                        {workingDaysInfo.publicHolidays > 0 && (
                          <span className="text-muted">
                            {workingDaysInfo.publicHolidays} holiday
                            {workingDaysInfo.publicHolidays !== 1 ? "s" : ""} excluded
                          </span>
                        )}
                      </div>
                      {workingDaysInfo.workingDays === 0 && (
          ...
          <div className="mb-3">
            ...
            <textarea
              ...
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder={isShort ? "Why do you need to change the time?" : "Why do you need to change the leave dates?"}
              disabled={submitting}
              style={{ fontSize: "0.9rem", resize: "none" }}
            />
          </div>
          ...
          >
            <Info size={13} className="flex-shrink-0 mt-1" />
            <span>
              Only <strong>future pending leaves</strong> can be edited.
            </span>
          </div>
        </div>
